...
def calcular_total(cargos, cantidades, vi, puntajes_dict, descuentos, antiguedad):
    ...
    for i, cargo in enumerate(cargos):
        if not cargo or cantidades[i] <= 0:
            continue
        puntaje = puntajes_dict.get(cargo, 0)
        transformacion += puntaje * cantidades[i] * vi * 1.23
    ...
    for i, cargo in enumerate(cargos):
        if not cargo or cantidades[i] <= 0:
            continue
    total_remunerativo = (
        basico + funcion_docente + antiguedad_valor +
        transformacion + adicional_jerarquico_total + bonificacion_total
    )
    zona = total_remunerativo
    total_remun = total_remunerativo + zona
