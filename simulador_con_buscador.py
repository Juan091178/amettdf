...
for i in range(3):
    ...
    with col1:
        entrada = st.text_input(f"Buscar cargo #{i+1} por código o nombre:", key=f"entrada_{i}")
        opciones_filtradas = df[df["IDENTIFICADOR"].str.contains(entrada, case=False, na=False)]["IDENTIFICADOR"].tolist()
        c = st.selectbox("Seleccionar cargo:", options=[""] + opciones_filtradas, key=f"selector_{i}")
    with col2:
