...
def calcular_horas_extra(total_horas, simples, completo, vi, antiguedad):
    ...
    zona = subtotal
    total_remun = zona * 2  # zona = 100%
    neto_unitario = total_remun * 0.81  # descuento 19%
...
if st.button("Calcular Comparación"):
    ...
    st.markdown(f"### 🕒 Horas Cátedra Extra en Abril: {cant_horas_extra} hora(s)")
    st.markdown(f"**Monto neto adicional por horas extra:** ${extra_total:,.2f}")
    st.markdown(f"**Total real cobrado en abril (con extras):** ${neto_abril_real:,.2f}")
    st.markdown(f"**📉 Diferencia real con mayo:** ${diferencia_real:,.2f}")
