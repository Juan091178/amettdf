...
if st.button("Calcular Comparación"):
    ...
    def resaltar_neto(row):
        return ['font-weight: bold; font-size: 1.1em;' if row['Concepto'] == 'Neto' else '' for _ in row]

    st.dataframe(
        df_resultado.style
            .apply(resaltar_neto, axis=1)
    ...
    st.markdown(f"<div style='font-size: 22px; font-weight: bold;'>Tu diferencia con el mes anterior es ${diferencia:,.2f}.</div>", unsafe_allow_html=True)
